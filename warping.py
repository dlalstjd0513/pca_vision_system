...
if __name__ == '__main__':
    print('ProGramStart')
    frame = cv2.imread('Image_20240618220520280.bmp')
    warp_result_frame = warp(frame)

    # Create a smaller custom window
    cv2.namedWindow('Warped Frame', cv2.WINDOW_NORMAL)
    cv2.resizeWindow('Warped Frame', 400, 300)  # Adjust the size as needed

    # Display the warped result frame
    cv2.imshow('Warped Frame', warp_result_frame)
